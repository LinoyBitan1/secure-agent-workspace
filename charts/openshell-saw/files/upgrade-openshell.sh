...
if [[ -n "${GATEWAY_IMAGE}" && -n "${SUPERVISOR_IMAGE}" && -n "${OPENSHELL_PIP_VERSION}" ]]; then
  ...
  # SSH and BOM commands resolve ~/.local/bin before /usr/local/bin. Refresh
  # those names from the native binaries just installed above; otherwise a
  # stale golden-image binary or version wrapper shadows the matching image.
  guest_ssh "
    install -d -m 755 /home/${SSH_USER}/.local/bin && \
    install -m 755 /usr/local/bin/openshell-gateway /home/${SSH_USER}/.local/bin/openshell-gateway && \
    install -m 755 /usr/local/bin/openshell-supervisor /home/${SSH_USER}/.local/bin/openshell-supervisor && \
    install -m 755 /usr/local/bin/openshell-supervisor /home/${SSH_USER}/.local/bin/openshell-sandbox
  " || echo "WARN: user-local OpenShell runtime refresh failed (non-fatal)"
  if [[ -n "${CLI_IMAGE}" ]]; then
    guest_ssh "
      install -m 755 /usr/local/bin/openshell /home/${SSH_USER}/.local/bin/openshell
    " || echo "WARN: user-local OpenShell CLI refresh failed (non-fatal)"
  fi
  guest_ssh "/usr/local/bin/openshell-gateway --version; /usr/local/bin/openshell-supervisor --version; openshell --version" || true
fi
